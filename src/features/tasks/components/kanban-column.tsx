...
export function KanbanColumn({ status, tasks, onTaskClick, onAddTask }: Props) {
  ...
  const meta = COLUMN_META[status] || { label: status, color: "text-zinc-400", dot: "bg-zinc-500" };
    ...
    <div className="flex flex-col min-w-[280px] w-[280px]">
      ...
      <div
        ...
        className={cn(
          ...
          isOver ? "border-violet-500/40 bg-violet-500/5" : "bg-zinc-900/30"
        )}
      >
        {tasks.map(task => (
        ...
        {onAddTask && status === "TODO" && (
          <button
            onClick={onAddTask}
            className="w-full text-left px-3 py-2 rounded-lg text-xs text-zinc-600 hover:text-zinc-300 hover:bg-zinc-800/50 transition-colors"
          >
            + Add Task
          </button>
        )}
