    ...
    <Popover>
      <PopoverTrigger asChild>
        ...
        >
          <Bell size={16} />
          {unreadCount > 0 && (
            <span className="absolute -top-0.5 -right-0.5 min-w-[16px] h-4 px-0.5 rounded-full bg-violet-500 text-white text-[9px] font-bold flex items-center justify-center leading-none">
      ...
      <PopoverContent
        align="end"
        side="right"
        className="w-80 p-0 bg-zinc-900 border border-zinc-800 shadow-2xl shadow-black/40 rounded-xl"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-800">
          ...
          {unreadCount > 0 && (
            <Button
              variant="ghost"
              size="sm"
              className="h-7 gap-1.5 text-xs text-zinc-400 hover:text-white px-2"
              onClick={markAllRead}
            >
              <CheckCheck size={12} />
              Mark all read
            ...
            <div className="p-2 space-y-0.5">
              ...
              {hasMore && (
                <Button
                  ...
                  onClick={loadMore}
                  disabled={isLoading}
                  className="w-full mt-1 text-xs text-zinc-500 hover:text-white"
                >
