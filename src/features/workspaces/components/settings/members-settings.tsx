    ...
    <div className="space-y-6">
      <Card className="border-zinc-800/60 bg-zinc-900/40 backdrop-blur-sm">
        <CardHeader className="flex flex-row items-start justify-between">
          <div>
            <CardTitle className="text-xl text-white">Workspace Members</CardTitle>
          ...
          <div className="rounded-md border border-zinc-800/60 overflow-hidden">
            <div className="divide-y divide-zinc-800/60">
              {members.map((m) => {
                ...
                return (
                  <div key={m.id} className="flex items-center justify-between p-4 bg-zinc-950/50 hover:bg-zinc-900/50 transition-colors">
                    ...
                    <div className="flex items-center gap-3 shrink-0 ml-4">
                      {hasPermission("member:change_role") && !isYou && !isTargetOwner ? (
                        <div className="flex items-center gap-2">
                          ...
                          >
                            <SelectTrigger className="w-[110px] h-8 bg-zinc-900 border-zinc-800 text-xs text-zinc-300">
                              <SelectValue />
                            </SelectTrigger>
                        ...
                        <div className="w-[110px] text-right">
                          <span className="text-xs font-medium text-zinc-500 capitalize">{m.role.toLowerCase()}</span>
                        </div>
                      )}
      ...
      <Dialog open={transferModalOpen} onOpenChange={setTransferModalOpen}>
        <DialogContent className="sm:max-w-[425px] bg-zinc-950 border-amber-900/30">
          <DialogHeader>
          ...
          <div className="py-4">
            <Select value={selectedNewOwner} onValueChange={setSelectedNewOwner} disabled={isTransferring}>
              ...
              <SelectContent className="bg-zinc-950 border-zinc-800 text-zinc-200 max-h-[200px]">
                {members.filter(m => m.userId !== currentMember.userId).map((m) => (
                  <SelectItem key={m.userId} value={m.userId}>
                    <div className="flex items-center gap-2">
                      <Avatar className="h-5 w-5 rounded-sm">
                        {m.user.image ? <AvatarImage src={m.user.image} /> : <AvatarFallback className="text-[10px] bg-zinc-800">{m.user.name?.charAt(0)}</AvatarFallback>}
                      </Avatar>
                      {m.user.name} <span className="text-zinc-500 ml-1">({m.user.email})</span>
                    </div>
                  </SelectItem>
                ))}
                {members.length === 1 && (
                  <div className="p-2 text-sm text-zinc-500 text-center">No other members available.</div>
                )}
              </SelectContent>
            </Select>
          </div>
          <DialogFooter className="gap-2 sm:gap-0">
            <Button variant="outline" onClick={() => setTransferModalOpen(false)} disabled={isTransferring} className="border-zinc-800 text-zinc-300 hover:bg-zinc-800">
              Cancel
            </Button>
            <Button
